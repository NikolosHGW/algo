...
Тут проблема была в том, что дубликаты я записываю из-за того, что двигаю два указателя, когда элементы равны. Из-за этого мой алгоритм вообще не знает о старых внесённых данных, то бишь может опять попасться тот элемент, который уже попал в результатирующий массив. Чтобы такой проблемы не было, мне нужно было всё-таки двигать только один указатель (второй), пока я не наткнусь на больший элемент во втором массиве
...
Всё-таки в предыдущей ошибки я ошибся насчёт того, чтобы двигать второй указатель, когда элементы равны. Получается, что надо двигать первый указатель, так как нам уже точно не надо никогда больше аппендить тот элемент, который был равен с тем, что во втором массиве
